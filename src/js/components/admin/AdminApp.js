...
function AdminApp() {
  ...
  const downloadProfile = (nextProfile) => {
    ...
    a.href =
      ...
      encodeURIComponent(JSON.stringify(nextProfile, null, 2));
  ...
  const uploadFile = () => {
    ...
    ConverterApi.fetchTables(selectedFile)
      ...
      .catch((error) => {
        if (error instanceof Error || error.status === 413) {
          setError(true);
  ...
  return (
    <Container fluid={["create", "update"].includes(status)}>
      <Breadcrumb className="mt-4">
        <Breadcrumb.Item onClick={showListView} active={status === "list"}>
      ...
      <Modal show={createdModal}>
        ...
        <Modal.Footer>
          <Button variant="primary" onClick={hideCreatedModal}>
            Great!
          </Button>
        </Modal.Footer>
      </Modal>
