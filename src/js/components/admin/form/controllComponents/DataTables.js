...
export default function OutputTables({ profile, setProfile, options }) {
  ...
  const updateAutomatedOperationDescription = (profile, index, key) => {
    ...
    if (profile.tables[index].table[key]) {
      ...
      for (const op of profile.tables[index].table[key]) {
        tmp_value.push(
          `${op.operator} [${type_mapping[op.type] ?? "??"}${additionalInfo(op)}]`,
        );
      }
  ...
  const toggleMatchTables = (index, op_index = -1) => {
    ...
    if (op_index === -1) {
      if (profile.matchTables) {
        // handling for old profiles
        profile.matchTables = false;
      } else {
        profile_table.matchTables = !profile_table.matchTables;
      }
    } else {
      profile_table.table.loop_metadata[op_index].ignoreValue =
        !profile_table.table.loop_metadata[op_index].ignoreValue;
    }
    setProfile(profile);
  };
  ...
  const updateTable = (index, key, value) => {
    if (index !== -1) {
      ...
      if (profile.tables[index].table[key] === null) {
        ...
        Object.values(profile.tables[index].table[key]).every(
      ...
      if (key === "DATA CLASS") {
        ...
        if (value === "XYDATA") {
          ["FIRSTX", "LASTX", "DELTAX"].forEach((headerKey) => {
            ...
            if (
              headerKey === key &&
              profile.tables[index].header[headerKey].type !==
                header[headerKey].type
            ) {
              header[headerKey] = initIdentifier(
                profile,
                header[headerKey].type,
              );
            }
  ...
  const loopMetadataOptions = (outputTable, op_index) => {
    ...
    const groups = tableMetadataOptions.reduce((acc, item, index) => {
      ...
      const showValue =
        !profile.tables[outputTable].table.loop_metadata[op_index]
          .ignoreValue && true;
  ...
  return (
    <>
        ...
        <Card key={index} className="mt-3">
          ...
          <Card.Body>
            ...
            <InputGroup>
              {profile.tables[index].loopType === "all" && (
                <InputGroup.Checkbox
                  ...
                  checked={
                    profile.matchTables ||
                    profile.tables[index].matchTables ||
                    false
                  }
              ...
              {profile.tables[index].loopType === "header" ? (
                <Button
                  ...
                  onClick={() => addOperation(index, "loop_header", "column")}
                >
                  +
                </Button>
              ) : (
                profile.tables[index].loopType !== "all" && (
                  <Button
                    ...
                    onClick={() =>
                      addOperation(
                        index,
                        `loop_${profile.tables[index].loopType}`,
                        profile.tables[index].loopType === "metadata"
                          ? "metadata"
                          : "header_value",
                      )
                    }
            ...
            {profile.tables[index].loopType !== "all" &&
              ...
              profile.tables[index].table["loop_header"].map(
                (operation, op_index) => (
                  <InputGroup key={op_index}>
                    ...
                    <Button
                      ...
                      onClick={() =>
                        removeOperation(index, "loop_header", op_index)
                      }
                    >
                      &times;
                    </Button>
            ...
            {profile.tables[index].loopType !== "all" &&
              ...
              profile.tables[index].table["loop_metadata"].map(
                (operation, op_index) => (
                  <InputGroup key={op_index}>
                    <InputGroup.Text>&#8627;</InputGroup.Text>
                    <Button
                      variant="outline-danger"
                      onClick={() =>
                        removeOperation(index, "loop_metadata", op_index)
                      }
                    >
                      &times;
                    </Button>
                    <Select
                      ...
                      openMenuOnScroll={false}
                      closeMenuOnScroll={false}
                      value={getSelectedMetadataOption(
                        operation.metadata,
                        index,
                        op_index,
                      )}
                      onChange={(selected) => {
                        ...
                        const selectedOption =
                          tableMetadataOptions[selected.value];
            ...
            {profile.tables[index].loopType !== "all" &&
              ...
              profile.tables[index].table["loop_theader"].map(
                (operation, op_index) => (
                  <InputGroup>
                    <InputGroup.Text>&#8627;</InputGroup.Text>
                    <Button
                      variant="outline-danger"
                      onClick={() =>
                        removeOperation(index, "loop_theader", op_index)
                      }
                    >
                      &times;
                    </Button>
            ...
            <TableForm
              ...
              updateOperation={(key, opIndex, opKey, value) =>
                updateOperation(index, key, opIndex, opKey, value)
              }
              updateOperationDescription={(key, value) =>
                updateOperationDescription(index, key, value)
              }
              removeOperation={(key, opIndex) =>
      ...
      <div className="mt-2">
        <Button variant="success" size="sm" onClick={() => addTable()}>
          Add table
        </Button>
      </div>
    </>
