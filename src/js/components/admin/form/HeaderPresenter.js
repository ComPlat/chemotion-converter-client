...
export default function FileHeaderPresenter({header, addIdentifier, updateRegex, profile, setProfile, tableIndex}) {
	...
	const setActive = (line, index) => {
		if (line) {
			for (let i = 0; i < seperatorList.length; i++) {
				if (line.split(seperatorList[i]).length === Number(profile.data.tables[tableIndex].metadata.columns)) {
					setSeperator(seperatorList[i]);
					setActiveLine(index);
				}
			}
		}
        ...
        <div ref={paragraphRef}>
          {header.map((line, index) => {
						return <React.Fragment key={index}>{(!multilineMode || index < multilineSelectionIndex) && (<>
							<span className={"code-line-index" + (activeLine === index ? " active" : "")} id={index}
										onMouseEnter={() => setActive(line, index)}
										onMouseLeave={() => setActiveLine(null)}
							>
								<Button size="sm" variant="info" ref={buttonRef}
												onClick={() => useAsColumnHeader(line)}>	&darr;</Button>
								<Overlay target={buttonRef.current} show={activeLine === index} placement="bottom">
									<Tooltip id="code-line-tooltip">
										Use this line as column header
									</Tooltip>
								</Overlay>
								<span>{index}</span>
							</span>
							<code data-idx={index}>{line}</code><br/>
						</>)}</React.Fragment>
					})}
        </div>
			{header.map((line, index) => {
