...
function TableColumn({
  ...
  return (
    <>
      ...
      {table[operationsKey] &&
        table[operationsKey].map((operation, index) => (
          <Row
            key={`operation-table-${operationsKey}-${index}`}
            className={
              "mb-2 align-items-end alert" +
              (is_extra_type(operation) ? " alert-warning" : "")
            }
          >
            {is_extra_type(operation) && (
              <p>
                Please note that, if the value is not available, the following
                calculation could result in an unexpected outcome!
              </p>
            )}
            <Col sm={2}>
              <OperatorSelect
                ...
                onChange={(value) =>
                  updateOperation(operationsKey, index, "operator", value)
                }
              />
            </Col>
              ...
              <Col sm={9}>
                {inputColumns.length > 0 ? (
                  <ColumnSelect
                    ...
                    columnList={inputColumns}
                    onChange={(column) =>
                      updateOperation(operationsKey, index, "column", column)
                    }
                  />
                ) : (
                  <ColumnInput
                ...
                <Col sm={9}>
                  ...
                  >
                    {inputTables.map((table, tableIndex) => (
                      <option key={tableIndex} value={tableIndex}>
                        Input table #{tableIndex}
                      </option>
                    ))}
                  </Form.Select>
                </Col>
            ...
            {operation.type === "metadata_value" &&
              tableMetadataOptions[operation.metadata] && (
                <Col sm={12}>
                  <Form.Check
                    inline
                    checked={operation.ignore_missing_values}
                  ...
                  <Form.Label column={true}>
                    <strong>Current Value:</strong>{" "}
                    {parseFloat(tableMetadataOptions[operation.metadata].value)}
                  </Form.Label>
                </Col>
              )}
          </Row>
      ...
      {table[`${operationsKey}Description`] &&
        table[`${operationsKey}Description`][0] && (
          <Form.Group controlId="operation-description" className="mt-3">
            <OverlayTrigger
              placement="bottom"
              overlay={
                <Popover id="metadata-popover">
                  <Popover.Header as="h3">
                    Description JCAMP comment{" "}
                  </Popover.Header>
                  <Popover.Body>
                    Here you can enter an explanation of how the mathematical
                    operations manipulate the data. This explanation is then
                    added to the JCAMP file as a comment.
                  </Popover.Body>
                </Popover>
              }
            >
        ...
        <OverlayTrigger
          ...
          overlay={
            <Popover id="metadata-popover">
              ...
              <Popover.Body>
                There is a risk of data corruption when using user-provided or
                free-text metadata for calculations. We recommend using only
                unmodifiable metadata, such as numerical result fields directly
                from the device.
              </Popover.Body>
            </Popover>
          }
