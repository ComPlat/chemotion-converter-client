...
function FileHeader({
  ...
  const { addIdentifier, updateRegex } = BuildIdentifierHandler(
    profile,
    setProfile,
    null,
  );
  return (
    <FileHeaderPresenter
      addIdentifier={(value) => {
        ...
        addIdentifier("tableHeader", true, {
          match: "regex",
          value,
          tableIndex,
        });
      ...
      {activeTable && (
        <div key={`tab-content-${activeKey}`}>
          ...
          {activeTable.metadata &&
            Object.keys(activeTable.metadata).length > 0 && (
              <div className="mt-3">
                ...
                <OverlayTrigger
                  ...
                  overlay={
                    <Popover id="header-popover-select-info">
                      ...
                      <Popover.Body>
                        To generate an identifier, please highlight the value
                        you wish to extract. Then press the “New Identifier”
                        button in the appearing dialog. Please remember to check
                        the regular expression and ensure it produces the
                        correct output.
                      </Popover.Body>
                    </Popover>
                  }
    ...
    <Col md={7}>
      {profile.data ? (
        <div className="scroll">
          ...
          {Object.keys(profile.data.metadata).length > 0 && (
            <Metadata metadata={profile.data.metadata}></Metadata>
          )}
          ...
          <TabContents
            ...
            profile={profile}
            setProfile={setProfile}
            activeTable={profile.data.tables[activeKey]}
            activeKey={activeKey}
          ></TabContents>
        </div>
      ) : (
