...
const fetchProperties = async (
  ...
  try {
    ...
    let result = data.response.docs
      .filter((item) => Boolean(item.obo_id))
      .map((item) => {
    ...
    if (additionalOptions) {
      ...
      const filteredAddedOptions = additionalOptions
        .filter((x) => x.obo_id.toLocaleLowerCase().includes(lInputValue))
        .map((x) => ontologySchemaToOption(x));
...
function OntologyAsyncSelect({
  ...
  return (
    <AsyncSelect
      ...
      defaultOptions
      loadOptions={(a) =>
        fetchProperties(a, create, typeFilter, additionalOptions)
      }
      onChange={(newValue) => {
        onChange(newValue);
      }}
        ...
        <div style={{ position: "relative", paddingRight: "40px" }}>
          <div style={{ display: "flex", flexDirection: "column" }}>
            ...
            >
              ...
              >
                {option.value.obo_id.split(":").at(-1)}
              </span>
            </div>
          </div>
        </div>
