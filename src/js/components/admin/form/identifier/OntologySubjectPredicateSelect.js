...
const OntologySubjectPredicateSelect = ({
  ...
  const mainForm = Object.entries(subjectInstances).map(
    ([subjectId, instances]) => {
      const subject = findOntologyById(subjectId, subjects);

      return (
        <div key={`ID_${subjectId}`}>
          <Row>
            <Col>
              <h6 className="mb-0 mt-3">{subject.label}</h6>
              <small style={{ display: "block", margin: "-2px 0 0 2px" }}>
                {subject.obo_id}
              </small>
            </Col>
          </Row>
          {instances.map((instance) => {
            return (
              <Row key={`${subjectId}__${instance.name}`}>
                <Col>
                  <Form.Group controlId={`${subjectId}__${instance.name}`}>
                    <Form.Label column="sm">
                      Predicate For <b>{instance.name}:</b>
                    </Form.Label>
                    <OntologyAsyncSelect
                      defaultOptions
  ...
  return (
    <Container>
      <Row>
        <Col>
          <p>
            The predicate for the subject instances specifies how the instances
            are connected to the actual measurement.
          </p>
        </Col>
      </Row>
      {mainForm}
